...
import cn.xyz.commons.KXMPPServiceImpl;
import cn.xyz.commons.KXMPPServiceImpl.MessageBean;
...
public class RoomManagerImplForIM implements RoomManager {
	...
	public void delete(User user, ObjectId roomId) {
		...
		dsForTigase.delete(dsForTigase.createQuery(Room.class).field("_id")
				.equal(roomId));
	}
	...
	@Override
	public List<Room> selectList(int pageIndex, int pageSize) {
		List<Room> roomList = dsForTigase.createQuery(Room.class)
				.offset(pageIndex * pageSize).limit(pageSize).order("-_id")
				.asList();
		return roomList;
	}
